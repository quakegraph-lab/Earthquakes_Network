...
def fault_zone_labels(
    ...
    detection method (a fair external ground truth, unlike the per-method
    cardinality match used in :func:`analyze_community_fault_alignment`).
...
def plot_fault_alignment(
    ...
    fig.suptitle(
        f"Community–fault alignment – {method_name} – {title}" if title
        else f"Community–fault alignment – {method_name}",
        fontsize=13, y=1.02)
    fig.tight_layout()
    if save:
